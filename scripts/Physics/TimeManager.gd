...
func _ready():
	if DEV_MODE:
		print("[TimeManager] DEV_MODE: TimeManager is ready!")
...
func _emit_initial_time_speed():
	...
	if DEV_MODE:
		print("[TimeManager] DEV_MODE: Initial time speed emitted after delay: ", global_time_speed)
...
func set_time_speed(new_speed: float):
	...
	emit_signal("time_speed_changed", global_time_speed)
	if DEV_MODE:
		print("[TimeManager] DEV_MODE: Time speed set to: ", new_speed)
		print("[TimeManager] DEV_MODE: Signal time_speed_changed emitted with value: ", new_speed)
